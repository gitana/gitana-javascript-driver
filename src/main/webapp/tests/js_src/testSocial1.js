(function($) {
    ...
    test("Comment operations", function() {
        stop();
        ...
        gitana.authenticate("admin", "admin").createRepository().readBranch("master").then(function() {
            ...
            // verify
            this.then(function() {
                this.subchain(node1).reload().then(
                        function() {
                            equal(this.get('stats')['ratingTotalCount'], 1, "Validate rating total count");
                            equal(this.get('stats')['ratingTotalValue'], 3, "Validate rating total value");
                            equal(this.get('stats')['ratingAverageValue'], 3, "Validate rating average value");
                        }).outgoingAssociations().count(function(count) {
                    ok(count > 0, "Number of outgoing associations from node 1 is > 0");
                });
                this.subchain(node2).reload().then(
                        function() {
                            equal(this.get('stats')['ratingTotalCount'], 1, "Validate rating total count");
                            equal(this.get('stats')['ratingTotalValue'], 4, "Validate rating total value");
                            equal(this.get('stats')['ratingAverageValue'], 4, "Validate rating average value");
                        }).outgoingAssociations().count(function(count) {
                    ok(count > 0, "Number of outgoing associations from node 1 is > 0");
                });
            });
